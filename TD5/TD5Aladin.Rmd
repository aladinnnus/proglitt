...
```{r}
df  %>% filter(preusuel=="MAXIME", annais!="XXXX") %>% mutate(nombre= as.integer(nombre),annais=as.integer(annais)) %>% group_by(preusuel,annais) %>%summarise(N=sum(nombre)) -> maximedf;
